...
interface Props {
  setBdcName: (name: string) => void; // <-- missing type definition
...
export default function File_selection_form({
  setBdcName,
  setFilingData,
...
}: Props)
 {
...
}, [data?.name]); // only re-run when data.name changes
  ...
  const onSubmit = async (values: z.infer<typeof FormSchema>) => {
    ...
    try {
      ...
      const primaryDoc = match.primaryDocument;

      // ✅ Correct SEC filing URL with primary document
      const secUrl = `https://www.sec.gov/Archives/edgar/data/${parseInt(
        cik
      )}/${accession}/${primaryDoc}`;

      const bdcName = values.selected_bdc.label;
      ...
      const response = await fetch(
        `${BASE_URL}fetch-free-sec-data?url=${encodeURIComponent(
          secUrl
        )}&bdc_name=${encodeURIComponent(bdcName)}`
      );
      ...
      if (result && result.tables) {
        setGroupedReports(result); // store entire response (with tables)
        toast.success("Financial tables loaded successfully!");
      } else {
        setGroupedReports(null);
        toast.error("No tables found in response.");
      }
  ...
  return (
                  ...
                  <PopoverTrigger asChild>
                    <FormControl>
                      <Button
                        ...
                        role="combobox"
                        className={cn(
                          "w-full justify-between bg-black",
    ...
    {/* Show BDC name from global state */}
      {data?.name && (
        <p className="text-base font-medium mt-2 text-[#435f97]">{data.name}</p>
      )}
    </>
  );
}
